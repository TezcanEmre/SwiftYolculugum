           ...
           let gelenVeriler = try context.fetch(fetchRequest) // gelen verileri atama yapıyoruz
            if gelenVeriler.count > 0 {
                for gelenVeri in gelenVeriler as! [NSManagedObject] { //gelen veri adında for döngüsü
                    ...
                    if let id = gelenVeri.value(forKey: "id") as? UUID { /** Coredata nın ayarladığı UUID verilerini NSobject den çıkarıp UUID olarak atama yapıyor */ idArray.append(id) }
        ...
        do {
            ...
            if gelenVeriler.count > 0 {
                for gelenVeri in gelenVeriler as! [NSManagedObject] {
                    if let id = gelenVeri.value(forKey: "id") as? UUID {
                        if id == idArray[indexPath.row] {
                            context.delete(gelenVeri)
                            dognameArray.remove(at: indexPath.row)
